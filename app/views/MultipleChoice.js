...
export class MultipleChoice extends React.Component {
  ...
  renderQuestions = () => {
    ...
    if (!questions || questions.length === 0) {
      navigation.navigate('QuizScreenRT');
      return;
    }
  ...
  calculateResult = () => {
    ...
    navigation.navigate('ResultScreenRT', { answers, score, time });
  };
  ...
  render() {
    ...
    return loading ? (
      ...
      <View style={styles.container}>
        ...
        >
          <Text> Question {`${asked + 1} of ${questions.length}`}</Text>
          <Text>
              ...
              {options.map((obj, i) => (
                <RadioButton labelHorizontal key={Math.random()}>
                  <RadioButtonInput
                    ...
                    buttonInnerColor="#fbcf3c"
                    buttonOuterColor={userAnswer === i ? '#fbcf3c' : '#000'}
                    buttonSize={40}
                    buttonOuterSize={55}
                  />
                  <RadioButtonLabel
